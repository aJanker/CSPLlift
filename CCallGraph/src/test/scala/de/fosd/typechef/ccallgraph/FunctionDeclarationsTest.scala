...
class FunctionDeclarationsTest extends TestHelper {
  ...
  @Test def testFunctionDefinitions(): Unit = {
    ...
    testFunctionDefs(
      """ void f() { }
        | int f() { } """
        .stripMargin, Set((("f", "function", 2), True)))

    testFunctionDefs(
      """#ifdef A
        | void f() { }
        | #endif """
        .stripMargin, Set((("f", "function", 2), FeatureExprFactory.createDefinedExternal("A"))))
    ...
    testFunctionDefs(
      """#ifdef A
        ...
        |   void f(int a) { }
        | #endif """
        .stripMargin, Set((("f", "function", 4), True)))


    testFunctionDefs("void g(int a, int b);", Set((("g", "declaration", 1), True)))
    testFunctionDefs("void g(int a, int b) {} ", Set((("g", "function", 1), True)))
  ...
  @Test def testNestedFunctionDefinitions(): Unit = {
    testFunctionDefs(
      """
        | void foo(char a, char b) {
        |   double bar(int z) { return z + 1; }
        |   return bar(a) + bar(b);
        | }
      """.stripMargin, Set((("foo", "function", 2),True), (("bar", "function", 3),True)))
    ...
    testFunctionDefs(
      """
        ...
        |   double bar(int z) { return z + 1; }
        |   #else
        |   double baz(int z) { return z-1; }
        |   #endif
        |   return bar(a) + bar(b);
        | }
      """.stripMargin, Set((("foo", "function", 2),True), (("bar", "function", 4), FeatureExprFactory.createDefinedExternal("A")), (("baz", "function", 6), FeatureExprFactory.createDefinedExternal("A").not())))
  }
