...
class ObjectNamesTest extends TestHelper {
    ...
    @Test def testAssignmentExpressions() {
        ...
        testExprStmt("a = &b;", Set("GLOBAL$a", "GLOBAL$b", "GLOBAL$&b"))
        testExprStmt("&a = b;", Set("GLOBAL$a", "GLOBAL$&a", "GLOBAL$b"))
        testExprStmt("y->a = x;", Set("GLOBAL$*y", "GLOBAL$y", "GLOBAL$y->a", "GLOBAL$x"))
        testExprStmt("y->a = &x;", Set("GLOBAL$*y", "GLOBAL$y", "GLOBAL$y->a", "GLOBAL$x", "GLOBAL$&x"))
        testExprStmt("y->a = *x;", Set("GLOBAL$*y", "GLOBAL$y", "GLOBAL$y->a", "GLOBAL$x", "GLOBAL$*x"))
        testExprStmt("a = &(&b);", Set("GLOBAL$a", "GLOBAL$b", "GLOBAL$&b", "GLOBAL$&(&b)"))
        testExprStmt("c = &(a+b);", Set("GLOBAL$a", "GLOBAL$&a", "GLOBAL$c"))
        testExprStmt("c = &(a+b+c);", Set("GLOBAL$a", "GLOBAL$&a", "GLOBAL$c"))
        testExprStmt("int *a; (((a->b)->c)->d) = a;", Set("foo$a", "foo$*a", "foo$a->b", "foo$*(a->b)", "foo$(a->b)->c", "foo$*((a->b)->c)", "foo$((a->b)->c)->d"))
        testExprStmt("a->b->c->d = a;", Set("GLOBAL$a", "GLOBAL$*a", "GLOBAL$a->b", "GLOBAL$*(a->b)", "GLOBAL$(a->b)->c", "GLOBAL$*((a->b)->c)", "GLOBAL$((a->b)->c)->d"))
    }

    @Test def testStatements() {
        testExprStmt("while(a) { }", Set("GLOBAL$a"))
        testExprStmt("while(*a) { }", Set("GLOBAL$a", "GLOBAL$*a"))
        testExprStmt("while(&a) { }", Set("GLOBAL$a", "GLOBAL$&a"))
        testExprStmt("while(a==1) { }", Set("GLOBAL$a"))

        // weird assignments but they can exist
        testExprStmt("while(a = 1) { }", Set()) // assignments with constants are ignored
        testExprStmt("while(a = b) { }", Set("GLOBAL$a", "GLOBAL$b"))
        testExprStmt("while(a = &b) { }", Set("GLOBAL$a", "GLOBAL$b", "GLOBAL$&b"))
        testExprStmt("while(a = p->x) { }", Set("GLOBAL$a", "GLOBAL$p", "GLOBAL$*p", "GLOBAL$p->x"))
        testExprStmt("while(a = &(p->x)) { }", Set("GLOBAL$a", "GLOBAL$p", "GLOBAL$*p", "GLOBAL$p->x", "GLOBAL$&(p->x)"))

        // n-ary-subexpressions are ignored (unless they contain assignments)
        testExprStmt("while (a == 1) { }", Set("GLOBAL$a"))
        testExprStmt("while (a == b) { }", Set("GLOBAL$a"))
        testExprStmt("while (a >= b) { }", Set("GLOBAL$a"))
        testExprStmt("while (a >= (b = c)) { }", Set("GLOBAL$a", "GLOBAL$b", "GLOBAL$c"))
        testExprStmt("while (a >= (b = &c)) { }", Set("GLOBAL$a", "GLOBAL$b", "GLOBAL$&c", "GLOBAL$c"))
        testExprStmt("while (a >= (b = *c)) { }", Set("GLOBAL$a", "GLOBAL$b", "GLOBAL$*c", "GLOBAL$c"))
        testExprStmt("while (a < b) { }", Set("GLOBAL$a"))
        testExprStmt("while (a || b) { }", Set("GLOBAL$a"))
        testExprStmt("while (a == &b) { }", Set("GLOBAL$a"))
        testExprStmt("while (a != p->x) { }", Set("GLOBAL$a"))
        testExprStmt("while (a != p->x) { }", Set("GLOBAL$a"))

        testExprStmt("for (a=1; b == p->x; p = p->y) { }", Set("GLOBAL$b", "GLOBAL$p", "GLOBAL$*p", "GLOBAL$p->y"))
        testExprStmt("for (a=1; b == p->x; p = p+c) { }", Set("GLOBAL$b", "GLOBAL$p"))
        testExprStmt("for (a=d; b != p->x; p = p->y) { }", Set("GLOBAL$a", "GLOBAL$d", "GLOBAL$b", "GLOBAL$p", "GLOBAL$*p", "GLOBAL$p->y"))
        testExprStmt("for (a=d; b < p->x; p = p->y) { }", Set("GLOBAL$a", "GLOBAL$d", "GLOBAL$b", "GLOBAL$p", "GLOBAL$*p", "GLOBAL$p->y"))
        testExprStmt("for (a=d, c=1; b = p->x; p = p+a) { }", Set("GLOBAL$a", "GLOBAL$d", "GLOBAL$b", "GLOBAL$p", "GLOBAL$*p", "GLOBAL$p->x"))

        testExprStmt("if (a) { }", Set("GLOBAL$a"))
        testExprStmt("if (a = 1) { }", Set())
        testExprStmt("if (a = b) { }", Set("GLOBAL$a", "GLOBAL$b"))
        testExprStmt("if (a == b) { }", Set("GLOBAL$a"))
        testExprStmt("if (a >= b) { }", Set("GLOBAL$a"))
        testExprStmt("if (a || b) { }", Set("GLOBAL$a"))
        testExprStmt("if (*a) { }", Set("GLOBAL$a", "GLOBAL$*a"))
        testExprStmt("if (a = p->x) { }", Set("GLOBAL$a", "GLOBAL$p", "GLOBAL$*p", "GLOBAL$p->x"))
        testExprStmt("if (a == (z = p->x)) { }", Set("GLOBAL$a", "GLOBAL$z", "GLOBAL$p", "GLOBAL$*p", "GLOBAL$p->x"))
        testExprStmt("if (a = p->x > z) { }", Set("GLOBAL$a", "GLOBAL$p", "GLOBAL$*p", "GLOBAL$p->x"))
        testExprStmt("if (1) { } else if (a) { } else { }", Set("GLOBAL$a"))
        testExprStmt("if (1) { } else if (a = 1) { } else { }", Set())
        testExprStmt("if (1) { } else if (a = b) { } else { }", Set("GLOBAL$a", "GLOBAL$b"))
        testExprStmt("if (1) { } else if (a == b) { } else { }", Set("GLOBAL$a"))
