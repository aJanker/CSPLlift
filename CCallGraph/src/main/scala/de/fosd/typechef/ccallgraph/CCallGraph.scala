...
class CCallGraph {
  ...
  def extractEdges() = {
    val functionDefsList = functionDefs.toPlainSet().map({ case (name, _, _) => name })
    ...
    for (((source, target), featExpr) <- functionCalls.toPlainSetWithConditionals()) {
      ...
      val functionTargets = target.split("\\|").filterNot({ s => s.equals("Empty") })
      ...
      for (functionTarget <- functionTargets) {
        ...
        if (!functionDefsList.contains(unscope(functionTarget))) {

          // search for equivalence class that contains the function call target
          var pointerEquivalenceClass = equivalenceClasses.find({ e => e.unscopedObjectNames().contains(functionTarget) })
          if (!pointerEquivalenceClass.isDefined) {
            pointerEquivalenceClass = equivalenceClasses.find({ e => e.unscopedObjectNames().contains("%s()".format(functionTarget)) })
          }
          ...
          if (!pointerEquivalenceClass.isDefined) {
            ...
            found = false
            for ((pointerTarget, pointerFeatExpr) <- pointerEquivalenceClass.get.objectNames.toPlainSetWithConditionals()) {

              // if target is function name
              if (functionDefsList.contains(unscope(pointerTarget))) {
  ...
  def createInitialPrefixSets(): Unit = {
    ...
    for ((o, o1) <- equivalenceClassesPrefixSets) {
      ...
      if (findObjectNameO.isDefined && findObjectNameO1.isDefined) {
        ...
        if ((uo.equals(apply(ObjectNameOperator.PointerCreation.toString, uo1))) || uo.equals(apply(ObjectNameOperator.PointerCreation.toString, parenthesize(uo1)))) {
        ...
        } else if ((uo.equals(apply(ObjectNameOperator.PointerDereference.toString, uo1)) || uo.equals(apply(ObjectNameOperator.PointerDereference.toString, parenthesize(uo1))))) {
        ...
        } else if ((uo.startsWith(apply(ObjectNameOperator.StructAccess.toString, uo1)) || uo.startsWith(apply(ObjectNameOperator.StructAccess.toString, parenthesize(uo1))))) {
        ...
        } else if ((uo.startsWith(apply(ObjectNameOperator.StructPointerAccess.toString, uo1)) || uo.startsWith(apply(ObjectNameOperator.StructPointerAccess.toString, parenthesize(uo1))))) {
          ...
          val eqClassObjectOPartial = equivalenceClassesMap.get(apply(ObjectNameOperator.PointerDereference.toString, objectNameFields._1))
