...
class CCallGraph {
  ...
  def checkObjectNamesEquivalenceClassConsistency(): Unit = {
    objectNames.keys.foreach({ k =>
      assert(find(k).isDefined == true)
    })
  }

  def find(objectName: ObjectName): Option[EquivalenceClass] = {
    for (equivalenceClass: EquivalenceClass <- equivalenceClasses) {
      if (equivalenceClass.find(objectName) || equivalenceClass.find(parenthesize(objectName))) {
        return Some(equivalenceClass)
      }
    }
    None
  }
    ...
    for ((o, o1) <- equivalenceClassesPrefixSets) {
      ...
      if (findObjectNameO.isDefined && findObjectNameO1.isDefined) {
        ...
        if (!findObjectNameO.isDefined) {
          println("Equivalence class not found for object name %s".format(o));
        } else if (!findObjectNameO1.isDefined) {
          println("Equivalence class not found for object name %s".format(o1));
        }
      }
    }
  }
