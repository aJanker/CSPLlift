    ...
    private def addGotoStatements(f: AST) {
        ...
        def get[T](a: Any)(implicit m: ClassTag[T]): List[Opt[T]] = {
            a match {
                ...
                case c: One[T @unchecked] if (m.runtimeClass.isInstance(c.value)) => List(Opt(FeatureExprFactory.True, c.value))
                case o: Opt[T @unchecked] if (m.runtimeClass.isInstance(o.entry)) => List(o)
