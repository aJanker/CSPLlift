...
class CPointerAnalysisContext(file: String) extends PointerContext with ObjectNameContext with EquivalenceContext with FunctionContext {
  ...
  private def saveContext(context: CPointerAnalysisContext, filename: String) =
    new ObjectOutputStream(new GZIPOutputStream(new FileOutputStream(filename + eqClassFileEnding))) {
      writeObject(context)
      close()
    }
